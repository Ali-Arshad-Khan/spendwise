...
export default function Expense() {
  ...
  const { groupedExpense, totalExpenseCount } = useTransactions();
  ...
  useEffect(() => {
    ...
    const formatted = Object.entries(monthlyTotals).map(([month, total]) => ({
      month,
      total,
  ...
  return (
    <div
      className="min-h-[100vh] h-full flex w-full bg-cover bg-center bg-mainbg bg-no-repeat"
      style={pp}
    >
          ...
          <div className="table-body text-white capitalize">
            {totalExpenseCount > 0 ? (
              ...
              <div className="empty-state flex normal-case justify-center items-center h-[30vh] px-5 text-center text-gray-400">
                <p>No expense transactions yet. Start by adding one!</p>
              </div>
            )}
          </div>
        </div>
